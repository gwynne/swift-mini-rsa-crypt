...
extension BoringSSLRSAPublicKey {
    fileprivate final class Backing {
        ...
        fileprivate var keySizeInBits: Int {
            return Int(CMiniRSACryptBoringSSL_RSA_size(self.pointer)) * 8
        }
    ...
    fileprivate final class Backing {
        ...
        private static func pkcs8DERPrivateKey<Bytes: ContiguousBytes>(_ derRepresentation: Bytes) -> UnsafeMutablePointer<RSA>? {
            return derRepresentation.withUnsafeBytes { derPtr in
                return BIOHelper.withReadOnlyMemoryBIO(wrapping: derPtr) { bio in
                    guard let p8 = CMiniRSACryptBoringSSL_d2i_PKCS8_PRIV_KEY_INFO_bio(bio, nil) else {
                        return nil
                    }
                    defer {
                        CMiniRSACryptBoringSSL_PKCS8_PRIV_KEY_INFO_free(p8)
                    }

                    guard let pkey = CMiniRSACryptBoringSSL_EVP_PKCS82PKEY(p8) else {
                        return nil
                    }
                    defer {
                        CMiniRSACryptBoringSSL_EVP_PKEY_free(pkey)
                    }
        ...
        fileprivate init(keySize: _RSA.Encryption.KeySize) throws {
            ...
            do {
                ...
                CMiniRSACryptBoringSSL_RSA_free(pointer)
                throw error
            }
        }
        ...
        fileprivate var publicKey: BoringSSLRSAPublicKey {
            let backing = BoringSSLRSAPublicKey.Backing(
                takingOwnershipOf: CMiniRSACryptBoringSSL_RSAPublicKey_dup(self.pointer)
            )
