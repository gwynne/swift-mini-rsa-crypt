...
#      a local copy of the BoringSSL sources in Sources/CMiniRSACryptBoringSSL.
#      Any prior contents of Sources/CMiniRSACryptBoringSSL will be deleted.
#
set -eou pipefail

HERE=$(pwd)
DSTROOT=Sources/CMiniRSACryptBoringSSL
...
function mangle_symbols {
    ...
    (
        ...
        xcodebuild -sdk iphoneos -scheme CMiniRSACryptBoringSSL -derivedDataPath "${TMPDIR}/iphoneos-deriveddata" -destination generic/platform=iOS
        ar -r "${TMPDIR}/libCMiniRSACryptBoringSSL-ios.a" "${TMPDIR}/iphoneos-deriveddata/Build/Products/Debug-iphoneos/CMiniRSACryptBoringSSL.o"
        (
            cd "${SRCROOT}"
            go run "util/read_symbols.go" -out "${TMPDIR}/symbols-iOS.txt" "${TMPDIR}/libCMiniRSACryptBoringSSL-ios.a"
        )
        ...
        (
            cd "${SRCROOT}"
            go run "util/read_symbols.go" -obj-file-format elf -out "${TMPDIR}/symbols-linux-all.txt" "${HERE}"/.build/*-unknown-linux-gnu/debug/libCMiniRSACryptBoringSSL.a
        )
    ...
    do
        $sed -i '1 i #define BORINGSSL_PREFIX CMiniRSACryptBoringSSL' "$assembly_file"
    done
...
(
    ...
    # - Their names will be prefixed with 'CMiniRSACryptBoringSSL_'
    # - The headers prefixed with 'boringssl_prefix_symbols' will also be prefixed with 'CMiniRSACryptBoringSSL_'
    ...
    # Now change the imports from "<openssl/X> to "<CMiniRSACryptBoringSSL_X>", apply the same prefix to the 'boringssl_prefix_symbols' headers.
    find . -name "*.[ch]" -or -name "*.cc" -or -name "*.S" | xargs $sed -i -e 's+include <openssl/+include <CMiniRSACryptBoringSSL_+' -e 's+include <boringssl_prefix_symbols+include <CMiniRSACryptBoringSSL_boringssl_prefix_symbols+' -e 's+include "openssl/+include "CMiniRSACryptBoringSSL_+'

    # Okay now we need to rename the headers adding the prefix "CMiniRSACryptBoringSSL_".
    pushd include
    find . -name "*.h" | $sed -e "s_./__" | xargs -I {} mv {} CMiniRSACryptBoringSSL_{}
    ...
    find . -name "*.h" | xargs $sed -i -e 's/include "/include "CMiniRSACryptBoringSSL_/' -e 's/include <CMiniRSACryptBoringSSL_\(.*\)>/include "CMiniRSACryptBoringSSL_\1"/'
    popd
)
...
#include "CMiniRSACryptBoringSSL_asn1_mac.h"
#include "CMiniRSACryptBoringSSL_asn1t.h"
#include "CMiniRSACryptBoringSSL_base.h"
#include "CMiniRSACryptBoringSSL_bio.h"
#include "CMiniRSACryptBoringSSL_blake2.h"
#include "CMiniRSACryptBoringSSL_blowfish.h"
#include "CMiniRSACryptBoringSSL_boringssl_prefix_symbols.h"
#include "CMiniRSACryptBoringSSL_boringssl_prefix_symbols_asm.h"
#include "CMiniRSACryptBoringSSL_cast.h"
#include "CMiniRSACryptBoringSSL_chacha.h"
#include "CMiniRSACryptBoringSSL_cmac.h"
#include "CMiniRSACryptBoringSSL_conf.h"
#include "CMiniRSACryptBoringSSL_cpu.h"
#include "CMiniRSACryptBoringSSL_curve25519.h"
#include "CMiniRSACryptBoringSSL_des.h"
#include "CMiniRSACryptBoringSSL_dtls1.h"
#include "CMiniRSACryptBoringSSL_e_os2.h"
#include "CMiniRSACryptBoringSSL_ec.h"
#include "CMiniRSACryptBoringSSL_ec_key.h"
#include "CMiniRSACryptBoringSSL_ecdsa.h"
#include "CMiniRSACryptBoringSSL_err.h"
#include "CMiniRSACryptBoringSSL_evp.h"
#include "CMiniRSACryptBoringSSL_hkdf.h"
#include "CMiniRSACryptBoringSSL_hmac.h"
#include "CMiniRSACryptBoringSSL_hrss.h"
#include "CMiniRSACryptBoringSSL_md4.h"
#include "CMiniRSACryptBoringSSL_md5.h"
#include "CMiniRSACryptBoringSSL_obj_mac.h"
#include "CMiniRSACryptBoringSSL_objects.h"
#include "CMiniRSACryptBoringSSL_opensslv.h"
#include "CMiniRSACryptBoringSSL_ossl_typ.h"
#include "CMiniRSACryptBoringSSL_pem.h"
#include "CMiniRSACryptBoringSSL_pkcs12.h"
#include "CMiniRSACryptBoringSSL_poly1305.h"
#include "CMiniRSACryptBoringSSL_rand.h"
#include "CMiniRSACryptBoringSSL_rc4.h"
#include "CMiniRSACryptBoringSSL_ripemd.h"
#include "CMiniRSACryptBoringSSL_rsa.h"
#include "CMiniRSACryptBoringSSL_safestack.h"
#include "CMiniRSACryptBoringSSL_sha.h"
#include "CMiniRSACryptBoringSSL_siphash.h"
...
module CMiniRSACryptBoringSSL {
    header "CMiniRSACryptBoringSSL.h"
    export *
}
EOF
